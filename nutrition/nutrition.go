...
func Eat(form url.Values) {
	...
	facts.Protein, _ = strconv.ParseFloat(form.Get("proteinInput"), 2)
	facts.Fat, _ = strconv.ParseFloat(form.Get("fatsInput"), 2)
	facts.Carbohydrate, _ = strconv.ParseFloat(form.Get("carbohydratesInput"), 2)
	facts.Calories, _ = strconv.ParseFloat(form.Get("caloriesInput"), 2)
	facts.Weight, _ = strconv.ParseFloat(form.Get("weightInput"), 2)
